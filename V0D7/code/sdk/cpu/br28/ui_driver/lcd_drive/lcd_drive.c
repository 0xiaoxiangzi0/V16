...
{
    printf("lcd_drv_backlight_ctrl_base value:%d.\n", percent);
    if (__lcd->backlight_ctrl) {
        __lcd->backlight_ctrl(percent);
    } else if (lcd_dat->pin_bl != -1) {
#if (TCFG_BACKLIGHT_PWM_MODE == 0)
        printf("11111111lcd_drv_backlight_ctrl_base value:%d, bl_pin:%d.\n", percent, lcd_dat->pin_bl);
        if (percent > 0) {
            gpio_set_direction(lcd_dat->pin_bl, 0);
