...
#if (TCFG_SMOKE_FUN_ENABLE)
static int smoke_int_handler()
{
	static u8 smoke_count = 0;
	smoke_count++;
	printf("============================smoke_int_handler count:%d.\n", smoke_count);
	app_task_put_key_msg(KEY_UI_SMOKING, 0);
}

void smoke_eint_config(void)
{
	gpio_set_pull_up(TCFG_SMOKE_EINT_PIN, 0);
	gpio_set_pull_down(TCFG_SMOKE_EINT_PIN, 1);
	gpio_set_die(TCFG_SMOKE_EINT_PIN, 1);
	gpio_direction_input(TCFG_SMOKE_EINT_PIN);

    port_edge_wkup_set_callback_by_index(5, smoke_int_handler); // 序号需要和板级配置中的wk_param对应上
    power_wakeup_index_enable(5, true);//中断使能
}
#endif
