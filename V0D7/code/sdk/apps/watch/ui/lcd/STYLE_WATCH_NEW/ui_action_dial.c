...
#define DIAL_SIDEBAR_START_THRESHOLD      (296 * 15 / 100)
#define DIAL_SIDEBAR_MAX_THRESHOLD      (296 * 70 / 100)//区域判断 
#define DIAL_SIDEBAR_MIN_THRESHOLD      (296 * 30 / 100)//区域判断 
#define DIAL_SIDEBAR_RUN_MAX_THRESHOLD  (296 * 85 / 100)//滑动距离判断
#define DIAL_SIDEBAR_RUN_MIN_THRESHOLD  (296 * 15 / 100)//滑动距离判断
#define DIAL_SIDEBAR_LEFT_MAX_THRESHOLD      (120 * 70 / 100)//区域判断
#define DIAL_SIDEBAR_LEFT_MIN_THRESHOLD      (120 * 30 / 100)//区域判断
#define DIAL_SIDEBAR_LEFT_RUN_MAX_THRESHOLD  (120 * 85 / 100)//滑动距离判断
#define DIAL_SIDEBAR_LEFT_RUN_MIN_THRESHOLD  (120 * 15 / 100)//滑动距离判断
#define DIAL_SIDEBAR_STEP               (296 * 20 / 100)//滑动步进 
...
enum SIDERBAR_STATUS {
    SIDEBAR_HIDE,            //重新开始页面操作步骤
    SIDEBAR_MOVE_X,          //
    SIDEBAR_MOVE_Y,          //
    SIDEBAR_SHOWING_TOP,     //正在从顶部展示（过程中），（可能是从表盘到下拉页面，也可能是从下拉页面到表盘，具体看实际情况）
    SIDEBAR_SHOWING_BOTTOM,  //正在从底部展示（过程中），（可能是从表盘到上拉页面，也可能是从上拉页面到表盘，具体看实际情况）
    SIDEBAR_SHOWING_LEFT,    //正在从左边展示（过程中），（可能是从表盘到左侧边栏，也可能是从左侧边栏到表盘，具体看实际情况）
    SIDEBAR_SHOW_TOP,        //从顶部展示（完成）：从下拉页面到表盘页面
    SIDEBAR_SHOW_BOTTOM,     //从底部展示（完成）：从上拉页面到表盘页面
    SIDEBAR_SHOW_LEFT,       //从左边展示（完成）
};

struct sidebar_priv {
    struct position pos;   //上一次触摸的x,y坐标
    int first_x_offset;    //第一次触摸按下时，x坐标
    int x_offset;          //本次与上一次触摸拖动的x坐标距离
    int first_y_offset;    //第一次触摸按下时，y坐标
    int y_offset;          //本次与上一次触摸拖动的y坐标距离
    int step;              //滑动步进值
    int timer;             //侧边栏定时器句柄
    u8 stop;               //true:当前静止状态    false：当前处于滑动状态
    enum SIDERBAR_STATUS sidebar_status;  //侧边栏状态
};
...
static int WATCH_ontouch(void *_ctrl, struct element_touch_event *e)
{
    ...
    struct rect rect;
    static int move_dir = SLIDER_MOVE_NONE;  //记录手滑动的方向
    ...
    case ELM_EVENT_TOUCH_MOVE:
        ...
        if (move_dir == SLIDER_MOVE_UD) {
            //根据手指滑动的距离，决定侧边栏显示的多少
            //首次按下时，处于顶部30%位置，则显示下拉页面
    ...
    case ELM_EVENT_TOUCH_U_MOVE:
        ...
        // UI_HIDE_CURR_WINDOW();
        // UI_SHOW_WINDOW(ID_WINDOW_QUICK_FUNC);
        break;
...
static int WATCH_onkey(void *ctr, struct element_key_event *e)
{
...
static int WATCH1_week_onchange(void *_number, enum element_change_event event, void *arg)
{
	u8 weed = 0;
	struct ui_pic *pic = (struct ui_pic *)_number;
	struct sys_time pTime;

	switch (event) {
	    case ON_CHANGE_INIT:
			get_sys_time(&pTime);
			//printf("===================befort week:%d================\r\n",weed);
			weed = rtc_calculate_week_val(&pTime);
			//printf("===================after week:%d================\r\n",weed);
...
REGISTER_UI_EVENT_HANDLER(WATCH1_WATCH1_WEEK)
.onchange  = WATCH1_week_onchange,
 .onkey = NULL,
  .ontouch = NULL,
};

static int WATCH1_pwr_onchange(void *_number, enum element_change_event event, void *arg)
{
...
#if 0
static int WATCH1_DATA_onchange(void *_ctrl, enum element_change_event event, void *arg)
{
...
REGISTER_UI_EVENT_HANDLER(WATCH1_WATCH1_DATE)
.onchange  = WATCH1_DATA_onchange,
 .onkey = NULL,
  .ontouch = NULL,
};
#endif
